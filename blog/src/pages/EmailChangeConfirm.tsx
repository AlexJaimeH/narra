  ...
  useEffect(() => {
    const confirmEmailChange = async () => {
      try {
        ...
        const response = await fetch(`/api/email-change-confirm?token=${token}`, {
          method: 'POST',
          headers: {
        ...
        if (response.ok && data.success) {
          setState('success');
          setMessage(data.message || 'Email cambiado exitosamente');
        ...
        >
          ...
          >
            ...
            <h1 className="text-3xl font-bold text-white">
              {state === 'loading' && 'Confirmando cambio...'}
              {state === 'success' && '✅ Cambio Confirmado'}
              {state === 'error' && '❌ Error'}
            </h1>
          </div>
              ...
              <div>
                ...
                <div className="space-y-4">
                  ...
                  <p className="text-center text-sm text-gray-500">
                    Ahora puedes iniciar sesión con tu nuevo email
                  </p>
                </div>
              </div>
            )}
            ...
            {state === 'error' && (
              <div>
                ...
                <div className="space-y-3">
                  ...
                  <p className="text-center text-sm text-gray-500">
                    Puedes intentar nuevamente desde tu configuración
                  </p>
                </div>
              </div>
            )}
