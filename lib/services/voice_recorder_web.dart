...
class _TranscriptAccumulator {
  ...
  bool apply(
    ...
    if (nextTranscript.startsWith(_value)) {
      final diffText = nextTranscript.substring(_value.length).trimLeft();
      if (diffText.isEmpty) {
        log('debug', 'Respuesta solo repite texto previo, se omite.');
        return false;
      }

      return appendAddition(diffText, log: log);
    }
    ...
    final diffText = nextTranscript.substring(prefixLength).trimLeft();
    if (diffText.isEmpty) {
