...
class VoiceRecorder {
  ...
  int _lastTranscribedByteLength = 0;
  ...
  Future<void> _transcribeLatest() async {
    ...
    if (!_stopping && audioBytes.length == _lastTranscribedByteLength) {
      _log('Ignorando transcripción: audio sin cambios', level: 'debug');
      return;
    }
    ...
    _lastTranscribedByteLength = audioBytes.length;
  }
  ...
  bool _applyTranscriptionPayload(Map<String, dynamic> payload) {
    ...
    if (segments is List) {
      ...
      if (seenIds.isEmpty) {
        ...
        final toRemove = <int>[];
        for (final key in _segments.keys.toList()) {
