...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  try {
    ...
    const model =
      ...
      normalizedPayload.model.trim().length > 0
    ...
    if (
      normalizedPayload.voice &&
      typeof normalizedPayload.voice === "string"
    ) {
    ...
    const clientSecret =
      typeof clientSecretRaw === "string"
        ? clientSecretRaw
        : typeof clientSecretRaw === "object" &&
            clientSecretRaw &&
            "value" in clientSecretRaw
    ...
    const expiresRaw =
      ...
      sessionJson.expiration_time ??
      sessionJson.expire_time ??
      null;
      ...
      expiresAt:
        expiresRaw == null || expiresRaw === ""
          ...
          : typeof expiresRaw === "string"
            ...
            : String(expiresRaw),
      clientSecret,
      iceServers: Array.isArray(sessionJson.ice_servers)
        ? sessionJson.ice_servers
        : [],
      model:
        typeof sessionJson.model === "string" && sessionJson.model.length > 0
          ? sessionJson.model
          : model,
    };
